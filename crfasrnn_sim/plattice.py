...
class HashTablePermutohedral(object):
    def __init__(self, kd_, vd_):
        ...
        self.keys = np.zeros((kd_ * self.capacity // 2), dtype='int32')
    ...
    def _grow(self):
        ...
        new_keys = np.zeros((self.kd * self.capacity // 2), dtype='int32')
...
class PermutohedralLattice(object):
    ...
    def __init__(self, d, vd, inp_len):
        ...
        self.greedy = np.zeros(self.d1, dtype='int32')
        self.rank = np.zeros(self.d1, dtype='int32')
        self.barycentric = np.zeros((d + 2), dtype='float32')
        self.replay = [{'offset': 0, 'weight': 0.} for _ in range(inp_len * self.d1)]
        self.nReplay = 0
        self.canonical = np.zeros((self.d1 ** 2), dtype='int32')
        self.key = np.zeros(self.d1, dtype='int32')
    ...
    @staticmethod
    def filter_3d(inp, ref, debug=True):
        """
        Filter the 3D image inp using the lattice defined by ref

        Attributes
        ----------
        inp : numpy array
            The 3D image to filter, should have the shape (depth, rows, cols, channels)
        ref : numpy array
        ...
        # Adjust lattice for 3D data
        lattice = PermutohedralLattice(ref.shape[-1], inp.shape[-1] + 1, inp.shape[0] * inp.shape[1] * inp.shape[2])
        ...
        # roll out ref to support running indexing for 3D data
        ref_channels = ref.shape[-1]
        ref = ref.flatten()
        ref_ptr = 0
        for d in range(inp.shape[0]):  # depth
            for r in range(inp.shape[1]):  # height
                for c in range(inp.shape[2]):  # width
                    col[:-1] = inp[d, r, c, :]
        ...
        # Slice from the lattice for 3D data
        if debug:
            run_times.append(time())
            log.info('Slicing...')
    ...
    def splat(self, position, pos_idx, value):
        ...
        self.greedy = np.where(v_ceil - self.elevated < self.elevated - v_floor, v_ceil, v_floor).astype('int32')
    ...
    def blur(self, reverse=False):
        ...
        """
        neighbour1 = np.zeros((self.d1), dtype='int32')
        neighbour2 = np.zeros((self.d1), dtype='int32')
